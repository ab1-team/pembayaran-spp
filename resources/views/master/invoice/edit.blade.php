...
<head>
    ...
    <title>Edit Invoice — {{ env('APP_NAME') }} Master</title>
...
<body class="text-slate-800">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <nav class="text-xs text-slate-500 mb-3">
            ...
            <span class="text-slate-700 font-medium">Edit #{{ $invoice->id }}</span>
        </nav>

        <div class="flex items-center justify-between gap-3 mb-6">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold text-slate-900">Edit Invoice</h2>
                <p class="text-sm text-slate-500">Update the selected invoice.</p>
            </div>
    ...
    <script>
        $(function () {
            ...
            @if (!empty($invoice->jumlah))
                $('.nominal').maskMoney('mask', {{ (float) $invoice->jumlah }});
            @endif
