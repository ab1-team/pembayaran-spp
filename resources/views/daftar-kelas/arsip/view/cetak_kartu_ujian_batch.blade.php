...
<head>
    <meta charset="UTF-8">
    <title>Kartu Peserta {{ $jenis_ujian ?? 'Ujian' }} - {{ $kelasLabel ?? '' }}</title>
    <style>
...
<body>
    ...
    @forelse ($siswaList as $row)
        @php
            $siswa = $row['siswa'];
            $noPeserta = trim(($siswa->nipd ?? '').' - '.$siswa->nisn, ' -');
        @endphp
        <div class="halaman">
            <div class="kartu">
                <div class="header">
    ...
    @empty
        <div class="text-center py-5">
            <h5>Tidak ada siswa yang memenuhi syarat cetak kartu.</h5>
            <p class="text-muted small">
                Syarat minimal bayar SPP untuk {{ $jenis_ujian }}: <strong>{{ $syarat }} bulan</strong>.
                Belum ada siswa di kelas ini yang mencapai syarat.
            </p>
        </div>
    @endforelse
